...
struct kmem_cache {
	...
	/* 标志 */
	unsigned long flags;
	/* 每个node结点中部分空slab缓冲区数量不能低于这个值，如果node结点中slab缓冲区数量大于这个值，那可以释放的空闲slab缓冲区将会释放，否则这些空闲slab缓冲区将不会释放 */
	unsigned long min_partial;
	/* 分配给对象的内存大小(可能大于对象的实际大小，包括对象后边的下个空闲对象指针) */
	...
	/* 保存slab缓冲区需要的页框数量的order值和objects数量的值，通过这个值可以计算出需要多少页框，这个是默认值 */
	...
	/* 缓冲区计数器，当用户请求创建新的缓冲区时，SLUB 分配器重用已创建的相似大小的缓冲区，从而减少缓冲区的个数。 */
	...
	/* 所有的 kmem_cache结构都会链入这个链表，链表头是 slab_caches */
