...
struct irq_data {
	...
	unsigned int		node;
	/* 硬件封装层需要使用的状态信息，不要直接访问该字段，内核定义了一组函数用于访问该字段 */
	unsigned int		state_use_accessors;
	/* 指向该irq所属的中断控制器的irq_chip结构指针 */
 ...
 */
enum {
	IRQD_TRIGGER_MASK		= 0xf,				/* 触发类型掩码 */
	IRQD_SETAFFINITY_PENDING	= (1 <<  8),		/* 挂起CPU亲和力设置(表明需要进行CPU亲和力设置) */
	IRQD_NO_BALANCING		= (1 << 10),		/* 禁止此IRQ在SMP系统中平衡到其他CPU，禁止此IRQ进行中断负载均衡 */
	IRQD_PER_CPU			= (1 << 11),			/* 相应的中断服务例程只在同一CPU上运行 */
	IRQD_AFFINITY_SET		= (1 << 12),			/* CPU亲和力已经设置 */
	IRQD_LEVEL			= (1 << 13),				/* 电平触发 */
	IRQD_WAKEUP_STATE		= (1 << 14),		/* 此IRQ发生中断能够唤醒系统 */
	IRQD_MOVE_PCNTXT		= (1 << 15),			/* 能够在处于进程上下文时产生中断 */
	IRQD_IRQ_DISABLED		= (1 << 16),			/* IRQ被系统禁用 */
	IRQD_IRQ_MASKED			= (1 << 17),		/* 此IRQ被屏蔽 */
	IRQD_IRQ_INPROGRESS		= (1 << 18), 		/* IRQ的一个处理程序正在执行 */
