...
/* ½ø³Ì×é£¬ÓÃÓÚÊµÏÖ×éµ÷¶È£¬¿ÉÄÜ»áÍ¬Ê±ÓĞÊµÊ±½ø³ÌºÍÆÕÍ¨½ø³Ì£¬Ò²»á¿ÉÄÜÍ¬Ê±ÔÚ²»Í¬CPUµÄÊµÊ±µ÷¶ÈºÍÆÕÍ¨µ÷¶ÈÖĞ½øĞĞ */
struct task_group {
	/* ÓÃÓÚ½ø³ÌÕÒµ½ÆäËùÊô½ø³Ì×é½á¹¹ */
	struct cgroup_subsys_state css;

#ifdef CONFIG_FAIR_GROUP_SCHED
	/* ÔÚ alloc_fair_sched_group() ÖĞ½ø³Ì³õÊ¼»¯¼°·ÖÅäÄÚ´æ */
	/* ¸Ã½ø³Ì×éÔÚÃ¿¸öCPUÉÏ¶¼ÓĞ¶ÔÓ¦µÄÒ»¸öµ÷¶ÈÊµÌå */
	struct sched_entity **se;
	/* ½ø³Ì×éÔÚÃ¿¸öCPUÉÏ¶¼ÓĞÒ»¸öCFSÔËĞĞ¶ÓÁĞ */
	...
	struct rcu_head rcu;
	/* ÓÃÓÚ½¨Á¢½ø³ÌÁ´±í(ÊôÓÚ´Ë½ø³Ì×éµÄ½ø³ÌÁ´±í) */
	struct list_head list;

	/* Ö¸ÏòÆäÉÏ²ãµÄ½ø³Ì×é£¬Ã¿Ò»²ãµÄ½ø³Ì×é¶¼ÊÇËüÉÏÒ»²ã½ø³Ì×éµÄÔËĞĞ¶ÓÁĞµÄÒ»¸öµ÷¶ÈÊµÌå£¬ÔÚÍ¬Ò»²ãÖĞ£¬µ÷¶È×éºÍ½ø³Ì±»Í¬µÈ¶Ô´ı */
...
/* CFSµ÷¶ÈµÄÔËĞĞ¶ÓÁĞ£¬Ã¿¸öCPUµÄrq»á°üº¬Ò»¸öcfs_rq£¬¶øÃ¿¸ö×éµ÷¶ÈµÄsched_entityÒ²»áÓĞ×Ô¼ºµÄÒ»¸öcfs_rq¶ÓÁĞ */
struct cfs_rq {
	 ...
	 * 2¡¢µ±ÈÎÎñ´Ó¶ÓÁĞÉ¾³ıÈ¥£¬ÈçÈÎÎñË¯Ãß»òÍË³ö£¬ÕâÊ±ºò»á²é¿´Ê£ÏÂµÄÈÎÎñµÄvruntimeÊÇ·ñ´óÓÚmin_vruntime£¬Èç¹ûÊÇÔò¸üĞÂ¸ÃÖµ¡£
	 */
	u64 min_vruntime;
#ifndef CONFIG_64BIT
	...
	/* ÏÂÒ»¸öµ÷¶È½áµã(ºìºÚÊ÷×î×ó±ß½áµã£¬×î×ó±ß½áµã¾ÍÊÇÏÂ¸öµ÷¶ÈÊµÌå) */
...
struct rq {
	...
	/* ¸ù¾İCPUÀúÊ·Çé¿ö¼ÆËãµÄ¸ºÔØ£¬cpu_load[0]Ò»Ö±µÈÓÚload.weight(SMPÊÇcfs.runnable_load_avg)£¬µ±´ïµ½¸ºÔØÆ½ºâÊ±£¬cpu_load[1]ºÍcpu_load[2]¶¼Ó¦¸ÃµÈÓÚload.weight(cfs.runnable_load_avg) */
...
#endif
	/* ÊÇ·ñĞèÒª¸üĞÂrqµÄÔËĞĞÊ±¼ä */
	int skip_clock_update;

	/* capture load from *all* tasks on this cpu: */
	/* CPU¸ºÔØ£¬¸ÃCPUÉÏËùÓĞ¿ÉÔËĞĞ½ø³ÌµÄloadÖ®ºÍ£¬nr_running¸üĞÂÊ±Õâ¸öÖµÒ²±ØĞë¸üĞÂ */
	struct load_weight load;
	/* ¸ºÔØ¸üĞÂ´ÎÊı */
	...
	/* µ±Ç°CPUËùÔÚ»ù±¾µ÷¶ÈÓò£¬Ã¿¸öµ÷¶ÈÓò°üº¬Ò»¸ö»ò¶à¸öCPU×é£¬Ã¿¸öCPU×é°üº¬¸Ãµ÷¶ÈÓòÖĞÒ»¸ö»ò¶à¸öCPU×Ó¼¯£¬¸ºÔØ¾ùºâ¶¼ÊÇÔÚµ÷¶ÈÓòÖĞµÄ×éÖ®¼äÍê³ÉµÄ£¬²»ÄÜ¿çÓò½øĞĞ¸ºÔØ¾ùºâ */
...
/* µ÷¶È×é£¬Ò²ÊÇCPU×é£¬°üÀ¨1¸ö»ò¶à¸öCPU */
struct sched_group {
	/* Ñ­»·Á´±í */
	struct sched_group *next;	/* Must be a circular list */
	atomic_t ref;
	/* ¸Ã×éÖĞËùÓĞCPUµÄ¸ºÔØÁ¿Ö®ºÍ */
	 ...
	 */
	/* ±£´æ´Ëµ÷¶È×éÖĞ°üº¬µÄCPUÑÚÂë£¬±íÃ÷µ±Ç°×éÓĞÄÄĞ©CPU */
...
/* ¸Ä±ä½ø³ÌËùÊôµÄÈÎÎñ¶ÓÁĞºÍµ÷¶ÈÊµÌå¸¸Ç× */
