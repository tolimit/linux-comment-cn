...
void resched_curr(struct rq *rq)
{
	 ...
	 * 标志是thread_info结构中的flags的TIF_NEED_RESCHED
	 */
	...
	if (cpu == smp_processor_id()) {
		...
		/* 设置CPU的内核抢占，就是清空了preempt_count中的PREEMPT_NEED_RESCHED */
...
{
	/* 判断需要加入到队列的进程是否为TASK_UNINTERRUPTIBLE进程 */
...
void set_task_cpu(struct task_struct *p, unsigned int new_cpu)
{
	...
	if (task_cpu(p) != new_cpu) {
		/* 当前CPU不为参数表示的CPU，将当前进程迁移到参数的CPU中 */
 ...
 */
/* update_process_times调用到，是时钟中断导致的调度tick */
 ...
 *	   如果一个新进程需要抢占当前进程，一般发生在以下情况中进行调度
 *
 *       - 开启了内核抢占
 *       - If the kernel is preemptible (CONFIG_PREEMPT=y):
 *         - 在系统调用或者异常中断上下文中，在调用preempt_enable()时
 *         - in syscall or exception context, at the next outmost
 *           preempt_enable(). (this might be as soon as the wake_up()'s
 *           spin_unlock()!)
 *         - 在中断上下文，从中断处理函数返回到可抢占上下文时(这里应该是中断下半部)
...
{
	/* 增加PREEMPT_ACTIVE计数，禁止了内核抢占 */
	__preempt_count_add(PREEMPT_ACTIVE);
	__schedule();
	/* 减少了PREEMPT_ACTIVE计数，如果PREEMPT_ACTIVE为0则开启了内核抢占 */
	__preempt_count_sub(PREEMPT_ACTIVE);
}
...
/*
 * CONFIG_FAIR_GROUP_SCHED: 公平组调度，建立于CFS调度基础上
 * CONFIG_RT_GROUP_SCHED: 实时组调度
 * CONFIG_CGROUP_SCHED: 控制组调度
 *
 * struct sched_entity作为一个调度对象(红黑树的一个结点)，它代表一组指定的进程。
 * 默认配置了 CONFIG_FAIR_GROUP_SCHED 和 CONFIG_RT_GROUP_SCHED
 *
 *
 */
