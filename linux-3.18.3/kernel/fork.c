...
/* 分配struct task_struct内存
 * orig: 父进程的struct task_struct
 * return: 新分配的struct task_struct
 */
static struct task_struct *dup_task_struct(struct task_struct *orig)
{
	struct task_struct *tsk;
	struct thread_info *ti;
	/* 获取父进程的信息(跟NUMA有关，没有配置NUMA则没什么用，返回-1) */
	int node = tsk_fork_get_node(orig);
	int err;

	/* 为新的进程分配struct task_struct结构体的内存，从slob中获取内存 */
	tsk = alloc_task_struct_node(node);
	if (!tsk)
		return NULL;

	/* 分配thread_info内存，内核选项可配置大小，默认是8K(两个连续的页)，包括新进程的内核栈。当设置为4K(一页)时中断和异常的保存位置会变化。 */
	ti = alloc_thread_info_node(tsk, node);
	if (!ti)
		goto free_tsk;

	/* 将父进程的task_struct所有值复制到子进程中 */
	err = arch_dup_task_struct(tsk, orig);
	if (err)
		goto free_ti;

	/* 设置新进程的内核栈为ti,thread_info与内核栈共用8K */
	tsk->stack = ti;
	...
	/* 根据父进程的 thread_info 设置子进程的 thread_info */
	setup_thread_stack(tsk, orig);
	clear_user_return_notifier(tsk);
	/* 清除此新进程需要调度标志，此标志一般在进程运行时需要换出时设置 */
	clear_tsk_need_resched(tsk);
	/* 设置栈底放溢出 magic(0x57AC6E9D) 数 */
	set_task_stack_end_magic(tsk);

	
#ifdef CONFIG_CC_STACKPROTECTOR
	/* 设置栈底的防溢出数 stack_canary */
	tsk->stack_canary = get_random_int();
#endif


	 /* tsk->usage = 2,使用计数设为2，一个是进程本身使用，一个是调用release_task()导致本进程退出的进程，所以为2 */
	...
	/* 暂时不明确其用途，大概是用于统计内核栈数量 */
	account_kernel_stack(ti, 1);

	/* 返回初始化好的 struct task_struct */
	return tsk;
...
static int dup_mmap(struct mm_struct *mm, struct mm_struct *oldmm)
{
	...
	rb_parent = NULL;
	/* 获取指向线性区对象的链表头，链表是经过排序的，按线性地址升序排列，但是mmap并不是list_head结构，是一个struct vm_area_struct指针 */
	pprev = &mm->mmap;
	/* 暂时不看，与ksm有关 */
	...
	for (mpnt = oldmm->mmap; mpnt; mpnt = mpnt->vm_next) {
		...
		*tmp = *mpnt;
		/* 初始化新的vma的anon_vma_chain为空 */
		INIT_LIST_HEAD(&tmp->anon_vma_chain);
		/* 视情况复制父进程vma的权限，非vm_flags */
		retval = vma_dup_policy(mpnt, tmp);
		if (retval)
			goto fail_nomem_policy;
		/* 将新的vma的vm_mm指向子进程的mm */
		tmp->vm_mm = mm;
...
/* 初始化这个子进程的mm_struct，这个函数是在父进程的上下文中执行的，p是子进程的task_struct */
static struct mm_struct *mm_init(struct mm_struct *mm, struct task_struct *p)
{
	mm->mmap = NULL;
	/* 为mm_struct新分配一个搜索vma的红黑色 */
	mm->mm_rb = RB_ROOT;
	mm->vmacache_seqnum = 0;
	/* mm_users和mm_count设为1 */
	atomic_set(&mm->mm_users, 1);
	atomic_set(&mm->mm_count, 1);
	/* 初始化读写信号量 */
	init_rwsem(&mm->mmap_sem);
	/* 初始化mmlist为空，mmlist用于连接当前进程的所有vma */
	...
	/* 清空cpumask，用于懒惰TLB */
	mm_init_cpumask(mm);
	/* 初始化异步IO用的数据，一个异步IO上下文链表，一个锁 */
	mm_init_aio(mm);
	/* mm->owner = 新的进程(当前运行的进程是父进程); */
	mm_init_owner(mm, p);
	/* 初始化通知链 */
	...
	/* 这个context是特定平台使用的结构，每个平台中此结构不一定相同 */
 ...
 */
/* 从slab中分配一个struct mm_struct内存出来，使用kmem_cache_alloc分配 */
 ...
 */
/* 递减mm_users，如果mm_users为0，这个函数就释放局部描述符表和线性区描述符及由内存描述符所引用的页表，并调用mmdrop()。在mmdrop()中把mm_count减1，如果mm_count为0，则释放mm_struct结构 */
...
/* 获取oldmm的可执行文件exe的struct file，让newmm->exe_file指向这个struct file */
...
static struct mm_struct *dup_mm(struct task_struct *tsk)
{
	...
	/* 为新进程的struct mm_struct分配内存，从mm_cachep这个slab中分配 */
	mm = allocate_mm();
	if (!mm)
		goto fail_nomem;

	/* 将父进程的mm_struct内容复制到新的mm_struct结构中 */
	memcpy(mm, oldmm, sizeof(*mm));

	/* 初始化mm_struct，因为当前的mm_struct中的内容是与父进程一模一样的 */
	if (!mm_init(mm, tsk))
		goto fail_nomem;

	/* 获取父进程的可执行文件exe的struct file，让子进程的exe_file指向这个struct file，表示当前执行的是这个文件 */
	      ...
	      int __user *child_tidptr)
{
	...
	/* 调用copy_process进行初始化，返回初始化好的struct task_struct结构体 */
