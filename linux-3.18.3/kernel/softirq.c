...
void irq_enter(void)
{
	...
	if (is_idle_task(current) && !in_interrupt()) {
		 ...
		 */
		/* 使用动态时钟时会执行这里，更新滞后的jiffies计数器，如果系统休眠，jiffies不会增加 */
		local_bh_disable();
		tick_irq_enter();
		_local_bh_enable();
	}
...
void raise_softirq(unsigned int nr)
{
	...
	/* 保存本CPU的IF标志并禁用本地CPU上的中断
	 * IF标志位表示CPU是否可以响应可屏蔽中断请求
	 */
	local_irq_save(flags);
...
/* 初始化软中断
 * softirq_vec是一个struct softirq_action数组，此结构体中只有一个action函数指针
 * nr代表软中断号，最大为NR_SOFTIRQS，10个左右
 */
