				...
				enum migrate_mode mode)
{
	 ...
	 * kicking migration.
	 */
	/* 判断是否支持大页移动，将此页从cc->migrate拿出来放回原来所在的地方 */
	if (!hugepage_migration_supported(page_hstate(hpage))) {
		putback_active_hugepage(hpage);
		return -ENOSYS;
	}

	/* 调用get_new_page，指向的是compaction_alloc
	 * 在这个函数里，首先检查cc中的freelist有没有空闲页框，如果有，则从空闲页框中获取一个页框，否则会从zone中扫描空闲页框
	 * 返回的是cc->freelist中第一个空闲页描述符
	 */
	...
	/* 成功取消了映射操作，这里将hpage复制到new_hpage */
