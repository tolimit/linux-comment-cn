 ...
 */
/* 释放线性地址空间 */
/* 两个阶段，1.把包含在start + len地址空间中的所有线性区从链表中解除连接
 * 2.更新进程的页表，并把第一阶段找到并标识出的线性区删除
 */
int do_munmap(struct mm_struct *mm, unsigned long start, size_t len)
{
	...
	/* 检查start是不是4KB倍数，是不是小于TASK_SIZE，以及start + len 是否小于 TASK_SIZE */
 ...
 */
/* 向内存描述符中插入一个线性区，增加mm_struct中的map_count */
