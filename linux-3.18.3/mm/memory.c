...
static void check_sync_rss_stat(struct task_struct *task)
{
	if (unlikely(task != current))
		return;
	/* 如果task->rss_stat.events大于TASK_RSS_EVENTS_THRESH阀值，将task->rss_stat.count数组清零 */
			     ...
			     unsigned long address, unsigned int flags)
{
	...
	/* 判断是否是大页的线性区，如果是则跳转到hugetlb_fault进行处理 */
	/* 判断方法 vma->vm_flags & VM_HUGETLB */
	if (unlikely(is_vm_hugetlb_page(vma)))
		return hugetlb_fault(mm, vma, address, flags);

	/* 获取页全局目录项 */
	pgd = pgd_offset(mm, address);
	/* 获取页上级目录项，如果没有则分配一个页作为页上级目录，并返回address对应的页上级目录项，并将页全局目录项与此页上级目录建立对应关系
	 * 为什么在用户空间要分配页上级目录的页，原因是当建立好线性区后，并不是立即给线性区中的地址做映射
	 * 而且访问到此线性区的具体地址时，才给此地址做映射关系，也就是初始化线性区时，并没做页表映射，对应的页上级目录，页中间目录和页表都为空
	 */
	pud = pud_alloc(mm, pgd, address);
	/* pud为空，则说明分配内存不成功 */
	if (!pud)
		return VM_FAULT_OOM;
	/* 同上，获取页中间目录项，如果没有则分配一个页作为页中间目录，并返回address对应的页中间目录项，并将页上级目录项与此页中间目录建立对应关系 */
	...
	if (pmd_none(*pmd) && transparent_hugepage_enabled(vma)) {
		/* 页中间目录项为空，vma是大页线性区并且vma不是临时栈。最重要一点是系统启用了transparent_hugepage，transparent_hugepage是让系统动态分配大页数量，而不是开机指定大小 */
		/* 为什么从这里要开始处理大页的情况，因为如果vma使用了大页的话，页表的页中间目录项与常规页表的页中间目录项不同 */
		int ret = VM_FAULT_FALLBACK;
		if (!vma->vm_ops)
			/* 如果vma->vm_ops没有具体的操作函数，则执行默认的do_huge_pmd_anonymous_page函数进行处理 */
		    ...
		    unsigned long address, unsigned int flags)
{
	 ...
	 */
	/* 如果是发生在用户态，则开启oom(out of memory)，oom会配合cgroup检查此进程使用的内存是否超过限定的内存大小，超过则会执行oom*/
