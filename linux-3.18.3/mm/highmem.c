...
/* 与pkmap_page_table配合使用，每个页表项有一个计数器
 * 计数器为0: 对应的页表项没有映射任何高端内存页框，并且可以使用
 * 计数器为1: 对应的页表项没有映射任何高端内存页框，但是它不能使用，因为自从它最后一次使用以来，其相应的TLB表项还未被刷新
 * 计数器为n: 相应的页表项映射一个高端内存页框，这意味着正好有n-1个内核成分在使用这个页框
 */
static int pkmap_count[LAST_PKMAP];
static  __cacheline_aligned_in_smp DEFINE_SPINLOCK(kmap_lock);

/* 专门的一个页表，用于高端内存永久映射，有512或者1024项页框，最多永久映射2MB或4MB高端内存 */
/* 该页表映射的线性地址从PKMAP_BASE开始，pkmap_count数字包含LAST_PKMAP个计数器，pkmap_page_table页表中每项都有一个这个计数器 */
...
{
	int i;
	/* 初始化page_address_htable[128]中所有的自旋锁 */
