 ...
 * directory entry:
 */
/* 页中间目录项pmd中对应的页表，如果页表不存在则申请一页用于新页表 */
 ...
 */
/* 初始化pgd_base指向的页全局目录中start到end这个范围的线性地址，整个函数结束后只是初始化好了页中间目录项对应的页表，但是页表中的内容并没有初始化 */
static void __init
page_table_range_init(unsigned long start, unsigned long end, pgd_t *pgd_base)
{
	...
	for ( ; (pgd_idx < PTRS_PER_PGD) && (vaddr != end); pgd++, pgd_idx++) {
		...
		for (; (pmd_idx < PTRS_PER_PMD) && (vaddr != end);
							pmd++, pmd_idx++) {
			/* 初始化整个页中间目录项和页表，必要时会为不存在的页表分配页框，不过页表初始化后是空的 */
 ...
 */
/* 建立页表，物理内存的第一个8MB已经在head.S中初始化好了 */
...
void __init mem_init(void)
{
	 ...
	 * important here.
	 */
	/* 所有高端内存管理区初始化，会将他们的页框回收到页框分配器中，并将所有node的所有zone的managed_pages置为0 */
	set_highmem_pages_init();

	/* this will put all low memory onto the freelists */
	/* 释放所有启动后不需要的bootmem表示的内存块到页框分配器 */
	free_all_bootmem();
